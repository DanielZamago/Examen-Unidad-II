...
function draw() {
    ...
    circuloPuntoMedio(circulo1X, circuloY, r);
    circuloPuntoMedio(circulo2X, circuloY, r);
    circuloPuntoMedio(circulo3X, circuloY, r);

    n = parseInt(prompt("Escribe cuantas lineas hacer en el circulo: "));
    angle = 2 * PI / n;

    for (let i = 0; i < n; i++) {
        let xLinea = circulo1X + r * cos(i * angle);
        let yLinea = circuloY + r * sin(i * angle);
        puntoA = new punto(circulo1X, circuloY);
        puntoB = new punto(xLinea, yLinea);
        drawPuntos(puntoPendiente(puntoA, puntoB));
    }
    for (let i = 0; i < n; i++) {
        let xLinea = circulo2X + r * cos(i * angle);
        let yLinea = circuloY + r * sin(i * angle);
        puntoA = new punto(circulo2X, circuloY);
        puntoB = new punto(xLinea, yLinea);
        lineasDDA(puntoA, puntoB);
    }
    for (let i = 0; i < n; i++) {
        let xLinea = circulo3X + r * cos(i * angle);
        let yLinea = circuloY + r * sin(i * angle);
        puntoA = new punto(circulo3X, circuloY);
        puntoB = new punto(xLinea, yLinea);
